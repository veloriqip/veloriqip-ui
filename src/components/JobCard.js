  ...
  return (
    <>
      <div className="w-full max-w-md border border-[rgb(var(--brand-navy))] bg-white shadow-lg rounded-lg p-6">
        ...
        <button key={props?.job.id} className="duration-500 mt-10 cursor-pointer px-4 py-2 bg-[rgb(var(--brand-navy))] text-white rounded-full hover:bg-[rgb(var(--btn-hover))]" onClick={() => router.push('/careers/'+ props?.job.id)}>
          Apply Now
        </button>
      </div>
    </>
